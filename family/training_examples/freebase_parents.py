...
def handle_person(request_id, response, exception):
    ...
    if not parents_query_id in result['output'][parents_query_id] or \
        not result['output'][parents_query_id]:
        return

    parents = [par['name'] for par in result['output'][parents_query_id][parents_query_id]]
    others = []
    ...
    try:
        ...
        if siblings_query_id.split('.')[1] in result['output'][siblings_query_id]:
            others += map(lambda per: per['name'], result['output'][siblings_query_id][siblings_query_id.split('.')[1]])
